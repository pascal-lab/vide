//! Resident slang elaboration service.
//!
//! This is a backend worker, not a cache. Slang's `Compilation` is not
//! incremental, so the value does not belong in salsa. One worker thread owns
//! the live compilations; queries name a snapshot revision and get a typed
//! result.
//!
...
use preproc_expand::compilation_plan::{self, compilation_source_buffers_for_plan};
...
fn compile_profile(db: &RootDb, profile_id: Option<CompilationProfileId>) -> Compilation {
    ...
    compilation.register_source_buffers(&compilation_source_buffers_for_plan(db, &plan));

    for file_id in &plan.roots {
        let path = compilation_plan::source_buffer_path(db, *file_id).to_string();
        let name =
            db.file_path(*file_id).map(|path| path.to_string()).unwrap_or_else(|| path.clone());
        match db.file_kind(*file_id) {
            base_db::source_db::SourceFileKind::LibraryMap => {
                compilation.parse_library_map_syntax_tree_from_buffer(
                    &name,
                    &path,
            ...
            _ => {
                compilation.parse_syntax_tree_from_buffer(
                    &name,
                    &path,
                    &SyntaxTreeOptions {
