...
use index::{
    ...
    SymbolNamespace, SymbolPath, SymbolPathComponent,
};
...
pub(crate) trait ProjectIndexDatabase {
    fn source_root_project_index(&self, source_root_id: SourceRootId) -> ProjectIndex;
    fn symbol_id_for_origin(&self, origin: DefinitionOrigin) -> Option<SymbolId>;
}

impl ProjectIndexDatabase for RootDb {
    fn source_root_project_index(&self, source_root_id: SourceRootId) -> ProjectIndex {
        let source_root = self.source_root(source_root_id);
        ProjectIndex::from_files(source_root.iter().map(|file_id| {
            let mut index = WorkspaceSymbolIndexDb::file_index(self, file_id).as_ref().clone();
            collect_source_occurrences(self, file_id, &mut index);
            index
        }))
    }

    fn symbol_id_for_origin(&self, origin: DefinitionOrigin) -> Option<SymbolId> {
        let name = origin.name(self)?;
    ...
    for offset in identifier_offsets(&text) {
        ...
        for token in target.into_tokens() {
            ...
            for origin in definition_class.origins() {
                ...
                let Some(symbol) = db.symbol_id_for_origin(origin) else {
                    continue;
                };
                index.occurrences.push(Occurrence {
                    symbol,
...
fn index_token_precedence(kind: syntax::TokenKind) -> usize {
    usize::from(kind.name_like())
}

fn identifier_offsets(text: &str) -> impl Iterator<Item = TextSize> + '_ {
...
mod tests {
    ...
    fn project_index_resolves_cross_file_module_instantiation_occurrence() {
        ...
        let project_index = db.source_root_project_index(root_id);
