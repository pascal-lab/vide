...
pub fn file_id_for_slang_path(db: &dyn SourceRootDb, slang_file: &str) -> FileId {
    for &file_id in db.files().iter() {
        if compilation_plan::source_buffer_path(db, file_id).as_str() == slang_file {
            return file_id;
        }
        if db.file_path(file_id).is_some_and(|path| path.as_str() == slang_file) {
            return file_id;
        }
    }
...
#[cfg(test)]
fn source_ast_ids_agree(text: &str, _name: &str, path: &str) -> (usize, usize) {
    let collect = |text: &str| {
        let (host, file_id) = crate::test_utils::setup_with_path(text, &format!("/{path}"));
        let db = host.raw_db();
        let tree = db.parse_src_for_compilation(file_id);
        let map = db.ast_id_map(file_id.into());
        let mut ids = Vec::new();
