// Thanks to rust-analyzer for this file.
...
pub struct LineIndex {
    /// Offset the beginning of each line (except the first, which always has
    /// offset 0).
...
impl LineIndex {
    ...
    /// Transforms the `LineCol` with the given `WideEncoding` into a
    /// `WideLineCol`.
    ...
    /// Transforms the `WideLineCol` with the given `WideEncoding` into a
    /// `LineCol`.
    ...
    pub fn range_for_line(&self, line: u32) -> Option<TextRange> {
    ...
    /// Returns the line for the given offset.
    pub fn line_for_offset(&self, offset: TextSize) -> Option<u32> {
        if offset > self.len {
            return None;
        }
        let line = self.newlines.partition_point(|&it| it <= offset) as u32;
        Some(line)
    }

    /// Given a range [start, end), returns a sorted iterator of non-empty
    /// ranges [start, x1), [x1, x2), ..., [xn, end) where all the xi, which
    /// are positions of newlines, are inside the range [start, end).
