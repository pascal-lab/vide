...
def reverse_maps(
    ...
    for ty_name, ty_info in all_types.items():
        ...
        while info.base != "SyntaxNode":
            kinds = reverse[current].copy()
            reverse[info.base].update(kinds)
...
def render_enum(
    ...
    def select_variant(kind: str) -> tuple[str, str | None]:
        candidates = sorted(
            base
            for base in bases.get(ty, set())
            if base in all_types and kind in reverse.get(base, set())
        )
        if not candidates:
            return kind, None
        base = candidates[0]
        if kind not in all_types and kind_map[kind] == ty:
            return kind, base
        return base, base

    for kind in kinds:
        variant, base = select_variant(kind)
        if base is not None and variant != kind and base in processed_bases:
            continue
        if base is not None and variant != kind:
...
def render_ast(all_types: dict[str, TypeInfo], kind_map: dict[str, str]) -> str:
    reverse, bases = reverse_maps(all_types, kind_map)
    parts = [
        ...
        "    }",
        "",
        "    pub fn syntax(self) -> SyntaxNode<'a> { self.parent }",
        "}",
        "",
        "#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]",
    ...
    for ty in sorted(name for name in all_types if name != "SyntaxNode"):
        ...
        if len(kinds) == 1:
            ...
            parts.append(render_enum(ty, all_types, reverse, bases, kind_map, members))
    return "\n".join(parts)
