...
def parse_subsystems(path: Path) -> list[str]:
    text = path.read_text()
    match = re.search(
        r"#define DS\(x\) \\\n(?P<body>.*?)\nSLANG_ENUM_SIZED\(DiagSubsystem",
        text,
        re.S,
    )
    if not match:
...
pub const DIAGNOSTIC_INFOS: &[DiagnosticInfo] = &[
{chr(10).join(rows)}
];
"""
...
def render_groups(
    groups: list[tuple[str, list[str]]], option_map: dict[str, list[str]]
) -> str:
    group_const_defs = []
    group_infos = []
    ...
    for name, options in sorted(groups):
        ...
        values = ", ".join(
            f"DiagCode::{screaming_snake(diag_name)}"
            for diag_name in sorted(diag_names)
        )
...
pub const DIAGNOSTIC_GROUPS: &[DiagnosticGroup] = &[
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate Rust diagnostic definitions from slang diagnostics.txt"
    )
    ...
    diagnostics_by_subsystem, diagnostics, option_map, groups = parse_diagnostics(
        args.input
    )
    missing_subsystems = sorted(set(diagnostics_by_subsystem) - set(subsystems))
    if missing_subsystems:
        raise RuntimeError(
            f"diagnostics.txt uses unknown subsystem(s): {missing_subsystems}"
        )
