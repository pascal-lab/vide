...
pub(super) fn collect_model(trace: Trace) -> Result<SourcePreprocModel, SourcePreprocError> {
    ...
    let mut model = SourcePreprocModel {
        root_source,
        sources,
        include_edges,
...
fn collect_trace_event(
    model: &mut SourcePreprocModel,
    source_order: usize,
    directive: Event,
) -> Result<(), SourcePreprocError> {
    model.inactive_ranges.extend(
        directive
            .disabled_ranges
            .iter()
    ...
    match kind {
        MacroEventKind::Define => {
            let event_index = model.defines.len();
            let define = collect_trace_define(directive, event_id, range);
            model.defines.push(define);
            push_source_event_record(model, event_id, kind, event_index, range);
        }
        MacroEventKind::Undef => {
            let event_index = model.undefs.len();
            model.undefs.push(SourceMacroUndef {
                event_id,
        ...
        MacroEventKind::Include => {
            let event_index = model.includes.len();
            let target = directive.include_file_name.include_target();
            model.includes.push(SourceMacroInclude {
                event_id,
                target,
        ...
        MacroEventKind::Conditional | MacroEventKind::Branch => {
            let event_index = model.conditionals.len();
            model.conditionals.push(SourceMacroConditional {
                event_id,
        ...
        MacroEventKind::Usage => {
            let event_index = model.usages.len();
            model.usages.push(SourceMacroUsage {
                event_id,
...
fn push_source_event_record(
    ...
    model.event_records.push(SourcePreprocEventRecord {
        event_id,
        kind,
        range,
        index: event_index,
