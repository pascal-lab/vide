...
有些故障是 Vide 本身的问题，这种情况欢迎通过 [GitHub Issues](https://github.com/pascal-lab/vide/issues) 向我们报告，并尽量附上可复现例子，具体信息见[故障报告](#故障报告)。
有些故障来自工作区或扩展配置，这种情况可以先按[常见问题和应对](#常见问题和应对)处理。
...
如果 `Vide Language Server` 输出通道里的信息还不够，或者问题只会在较长流程里出现，再打开更详细的文件日志。在 `vide.server.additionalArgs` 中加入 `--log` 和 `--log_file`，然后重启语言服务器：
...
如果服务器本身还启动不了，直接看下面的“扩展或自定义服务器无法启动”。
...
- `Bundled Vide Language Server binary not found` 或 `Unsupported platform-architecture combination`：
  先核对安装的 VSIX 和当前平台是否匹配。如果你是通过本地打包安装的 VSIX，需要确认打包时运行的是 `npm run package:vsix` 或 `npm run package:vsix:debug`。这些命令会把语言服务器二进制打进 VSIX；单独执行 `npm run compile` 只会编译扩展前端，安装后会缺少服务器。
...
这一节用于排查“VS Code 扩展启动不了内置服务器”或“配置了自定义 `vide.server.command` 后启动不了”的情况。

先确认 VS Code 扩展实际用什么命令启动语言服务器。执行 `Vide：显示状态`、`Vide：显示语言服务器输出` 和 `Vide：显示服务器版本`，重点看下面几项：
...
如果 `Vide：显示服务器版本` 也失败，说明扩展当前使用的服务器命令、工作目录或基础参数还不能正常运行。
...
确认方式是：用上面看到的 `Server command` 和参数在终端里跑一次。如果你配置了自定义服务器，再继续检查：
...
这一节用于排查“新增、删除或修改工程文件后，Vide 没有重新加载项目”的情况。

默认 `vide.files.watcher` 是 `client`。这表示 VS Code 扩展会优先把 VS Code 收到的文件变化事件转发给 Vide 语言服务器。如果当前编辑器环境不支持动态监听文件，Vide 会自动改用语言服务器进程自己的文件监听器。

如果工程文件变化后没有触发刷新，可以先临时切到语言服务器进程自己的监听器：
...
`vide.files.excludeDirs` 是 VS Code 扩展设置，用于告诉 Vide 忽略哪些工作区相对目录。它只接受目录名，不支持 `**` 这类 glob 写法。

选择“哪些文件属于项目”时，优先写 `vide.toml` 的 `sources` 和 `exclude`。如果你还想让 VS Code 自己减少对某些大目录的文件监听事件，再另外配置 VS Code 的 `files.watcherExclude`。
