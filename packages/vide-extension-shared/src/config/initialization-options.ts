...
function setting<T>(
  config: ConfigurationReader | undefined,
  section: string,
  fallback: T,
): T {
...
export function videInitializationOptions(
  ...
  return {
    ...
    workspace: {
      auto: {
        reload: setting(config, "workspace.auto.reload", true),
      },
    },
    scope: {
    ...
    formatting: {
      on: {
        enter: setting(config, "formatting.on.enter", true),
      },
      in: {
        comments: setting(config, "formatting.in.comments", true),
      },
      indent: {
        width: setting(config, "formatting.indent.width", 4),
      },
    },
    inlayHints: {
      port: {
        connection: {
          enable: setting(config, "inlayHints.port.connection.enable", true),
        },
      },
      parameter: {
        assignment: {
          enable: setting(config, "inlayHints.parameter.assignment.enable", true),
        },
      },
      end: {
        structure: {
          enable: setting(config, "inlayHints.end.structure.enable", true),
        },
      },
    },
    lens: {
      instantiations: {
        enable: setting(config, "lens.instantiations.enable", true),
      },
    },
    semantic: {
      tokens: {
        port: {
          clk: {
            rst: {
              enable: setting(config, "semantic.tokens.port.clk.rst.enable", true),
            },
          },
          input: {
            output: {
    ...
    diagnostics: {
      ...
      parse: {
        enable: setting(config, "diagnostics.parse.enable", true),
      },
      semantic: {
        enable: setting(config, "diagnostics.semantic.enable", true),
      },
      slang: {
    ...
    signature: {
      help: {
        params: {
          only: setting(config, "signature.help.params.only", false),
        },
      },
    },
    qihe: {
...
export function videDiagnosticsProfilingInitializationOptions(
  config?: ConfigurationReader,
): Record<string, unknown> {
  const options = videInitializationOptions(config);
