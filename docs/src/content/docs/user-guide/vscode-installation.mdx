...
<ThinLinkCard
  ...
  action="打开"
>
  安装稳定版 Vide，扩展 ID：<code>pascal-lab.vide-code-extension</code>
</ThinLinkCard>

也可以在 VS Code 的扩展面板搜索显示名 `Vide`，或搜索扩展 ID `pascal-lab.vide-code-extension`。
...
扩展激活后，VS Code 底部状态栏会出现名为 `Vide` 的状态项。正常启动后状态项显示 `Vide`。此时的状态栏有可能显示为黄色，因为此时没有创建项目配置文件 `vide.toml`。

![status bar](../assets/quick-start/status-0.png)

而如果妥善配置了 `vide.toml`，把鼠标悬停在状态项上，就会看到下图所示的状态详情。

![status bar](../assets/quick-start/status-1.png)

两种状态都表示 Vide 已启动，可以继续体验。
