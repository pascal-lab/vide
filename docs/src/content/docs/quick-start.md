...
用 VS Code 打开包含 RTL 源码的目录。没有 `vizsla.toml` 或 `vizsla_config.toml` 时, 扩展会创建默认 `vizsla.toml` 并弹出提示:
...
需要跨文件语义诊断、跳转和端口/参数相关能力时, 请在工程清单中写入实际的 `sources` 或 `include_dirs`, 并按需补充 `defines`, `libraries` 或 `top_modules`。
