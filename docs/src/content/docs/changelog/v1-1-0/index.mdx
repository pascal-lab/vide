...
在 VS Code 里，它们通常出现在灯泡菜单；在其他编辑器里，可以从代码动作或 Code Action 入口调用。
...
- 自增、自减和赋值写法转换：`展开后缀表达式`、`展开前缀表达式`、`将后缀表达式转换为前缀表达式`、`将后缀表达式转换为复合赋值`、`将前缀表达式转换为后缀表达式`、`将前缀表达式转换为复合赋值`、`将复合赋值转换为后缀表达式`、`将复合赋值转换为前缀表达式`、`将赋值转换为后缀表达式`、`将赋值转换为前缀表达式`。 ([#211](https://github.com/pascal-lab/vide/pull/211))
