...
- 新增更多 Code Actions：`always` / `always_comb` / `always_ff` 转换、ANSI / non-ANSI 端口声明转换、命名端口简写展开/折叠、去除多余括号、合并嵌套 `if`、提取变量、`if` 语句和条件表达式 `?:` 转换、数字字面量重排等。([#239](https://github.com/pascal-lab/vide/pull/239) by [@roife](https://github.com/roife))
