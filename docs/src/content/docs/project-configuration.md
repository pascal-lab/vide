...
Vizsla 的工程清单文件名推荐使用 `vizsla.toml`, 也兼容旧名 `vizsla_config.toml`。它必须放在 workspace root 下, 或者被显式作为工程路径传入。字段名必须使用当前支持的名字, 旧字段或未知字段会被拒绝。如果两个文件同时存在, Vizsla 优先读取 `vizsla.toml`。
...
- 如果库目录下有 `vizsla.toml` 或 `vizsla_config.toml`, 我们会按这个清单加载库。
...
- 清单文件名推荐使用 `vizsla.toml`, 旧名 `vizsla_config.toml` 仍然兼容。
- `top_module`, `include`, `macros` 这类旧字段不会被接受。
- 我们不会从子目录自动发现清单。打开 `repo` 时, 只检查 `repo/vizsla.toml` 和 `repo/vizsla_config.toml`。
