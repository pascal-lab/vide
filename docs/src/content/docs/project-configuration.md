...
这个默认清单会让 Vizsla 以 best-effort 方式索引 workspace 下的 Verilog/SystemVerilog 文件, 用于跳转、引用等读能力, 但不会建立编译 profile 或运行跨文件 semantic diagnostics。Best-effort 索引遇到重名 module 时只会在唯一最近候选存在时按路径距离推测; 配置化工程不会把目录当作隐式命名空间。显式写入 `sources = []` 会关闭 workspace 索引, 回到只处理打开文件的低成本模式。需要更准确的 semantic diagnostics 时, 请写入符合工程结构的 `sources` shell glob 或 `include_dirs`, 并按需补充 `defines`, `libraries` 或 `top_modules`。
