...
Semantic diagnostics 需要 workspace root 下的 `vizsla.toml` 或 `vizsla_config.toml` 配置可加载的工程内容, 例如实际的 `sources` 或 `include_dirs`; `defines`, `libraries` 和 `top_modules` 可以在此基础上补充工程信息。VS Code 扩展会在缺少清单时创建 `sources = []` / `include_dirs = []` 的默认清单; 这个默认清单和无清单一样只保留打开文件的 syntax/parse diagnostics。
...
如果跳转结果不符合预期, 先检查工程是否加载到了目标文件, 以及 workspace root 下是否有工程清单。更复杂目录建议显式配置 `sources`, `include_dirs` 和 `libraries`。
