...
### Generate qihe-options.toml

If you want to use a Qihe options file, run `Vide: Generate Qihe Options File` from the Command Palette. Vide runs this command in the current workspace root:

```bash
qihe gen-options -a -o ./qihe-options.toml
```

If `qihe-options.toml` already exists, Vide offers to open the existing file and does not overwrite it silently. Later runs of `Vide: Run Qihe Analysis` automatically use the file from the workspace root.

A common use is to configure a stable Qihe storage directory:
...
Vide reads Qihe diagnostics from this `storage.root`. If the options file does not define `storage.root`, Vide still creates a temporary storage directory for that run.
...
qihe run <qihe-run-args> [--options ./qihe-options.toml] -i <tmp.qh> [-c storage.root=<tmp-storage>]
```

Parameters in brackets are only added when needed.
...
- `--options ./qihe-options.toml` is added when `qihe-options.toml` exists in the current workspace root.
- `<tmp.qh>` is a temporary IR path created by Vide for the run.
- `-c storage.root=<tmp-storage>` is added when no usable `storage.root` is available; `<tmp-storage>` is a temporary diagnostics storage path created by Vide for the run.
...
`<input-files>` means the Verilog/SystemVerilog input files passed to `qihe compile`. `vide.qihe.autoConfigureArgsFromManifest` only controls whether arguments such as `--top`, `-I`, and `-D` are added automatically. It does not change which files Vide analyzes: when `vide.toml` is configured, Vide analyzes the project's source files; when no usable project configuration is available, Vide analyzes only the currently open file.
...
If you are not sure where an option belongs, use its purpose as the guide: options that affect how source files are compiled or parsed go in `vide.qihe.compileArgs`; options that affect which Qihe analyses run or how they run go in `vide.qihe.runArgs`. Command arguments are documented here:
