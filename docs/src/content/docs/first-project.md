...
如果 VS Code 打开的 workspace root 下没有 `vizsla.toml` 或 `vizsla_config.toml`, 扩展会创建默认 `vizsla.toml` 并弹出提示:
...
如果通过其它客户端或命令行方式启动服务器, 且确实没有工程清单, Vizsla 只保留 syntax/parse diagnostics, 不运行 semantic diagnostics。
...
清单只会从你打开的 workspace root 读取。我们不会自动向父目录或子目录搜索其它 `vizsla.toml` 或 `vizsla_config.toml`。
