...
| `IndexOnly` | 只做 best-effort 索引, 不进入编译 profile。 |
| `Library` | 依赖库 root, 会参与引用工程的 semantic profile。 |
| `Ignored` | 不参与解析、索引或诊断。 |

`IndexOnly` 是默认可读性的关键: 它让跳转、引用等功能尽量能用, 但不会把这些文件假装成一个准确的编译工程。
...
如果当前文件只属于默认 `IndexOnly` root, 或者没有可用的项目编译 root, Vizsla 会回到单文件输入。这样默认索引不会误触发项目模式。
