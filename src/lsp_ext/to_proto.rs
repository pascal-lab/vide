...
pub(crate) fn inlay_hint(
    ...
    let label = lsp_types::InlayHintLabelPart {
        ...
        location: target_location.and_then(|InFile { value, file_id }| {
            let file_range = FileRange { file_id: file_id.expect_file(), range: value };
    ...
    let kind = match kind {
        InlayKind::ParamAssign
        | InlayKind::Port
        | InlayKind::MacroArgument
        | InlayKind::SystemCall => Some(lsp_types::InlayHintKind::PARAMETER),
